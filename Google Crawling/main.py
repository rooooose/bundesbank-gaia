...
def find_links(last_comp_index):
    year_changed = False
...
def download_from_doubt_links(year, company):
    is_doubt = False
...
def download_from_found_links(year, company, is_doubt):
...
def download_read_pdfs(last_comp_index):
    year_changed = False
    for year in years_to_search[last_year_index:]:
        ...
        for company in companies[last_comp_index+1:]:
            ...
            is_doubt = download_from_doubt_links(year, company)
            download_from_found_links(year, company, is_doubt)
...
download_read_pdfs(last_comp_index)
